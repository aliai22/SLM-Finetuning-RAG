...
from llmrag.pipelines.base_rag2.rag_main import run_rag_pipeline

def main(argv=None):
    p = argparse.ArgumentParser()
    p.add_argument("--eval-json", default=".data/finetuning/qa/synthetic_QAs.json")
    p.add_argument("--out-csv", default=".artifacts/logs/rag_eval_results_b2.csv")
    p.add_argument("--max-examples", type=int, default=None)
    args = p.parse_args(argv)

    qa = prepare_rag_evaluation_data(file_path=args.eval_json)
    print(f"[b2] Total QA Pairs: {len(qa)}")
